...
---
#### split
Split enables detailed segmentation of data along specific dimensions. By splitting raw event data into meaningful subsets, the function facilitates granular analysis and visualization. This functionality is particularly useful when working with complex, high-volume and variety datasets like AWS CloudTrail events.
...
// Split data by relevant dimensions
.split($raw_events; dimension='EventName'; dimension='Username'; dimension='Region'; annotation='CloudTrailEvent').as($segmented_events)
...
- parameters: 1 time series[] or 1 aggregate[] variable and 1 integer flag.
...
- parameters: 1 environment reference
- returns: 
- use:
...
#### cloudwatch
Multiple cloudwatch metric data sources can be configured in the environment definition. 
- attributes:
...
```
#### cloudwatchlogs
Multiple cloudwatch log data sources can be configured in the environment definition. 
- attributes:
	- name: The name that will be referenced by the "where" function in a PQL program.
	- type: cloudwatchLogs
	- vars:
...
```
#### sentry
Multiple Sentry event sources can be configured in the environment definition. 
- attributes:
	- name: The name that will be referenced by the "where" function in a PQL program.
	- type: sentry
	- vars:
		- organization: The org name of the Sentry account we are fetching events from.
		- access_token: API access token provided by Sentry.
```
{
      "name": "sentry_demo",
      "type": "sentry",
...
```
#### cloudtrail
Multiple cloudtrail data sources can be configured in the environment definition. 
- attributes:
	- name: The name that will be referenced by the "where" function in a PQL program.
	- type: cloudtrail
	- vars:
